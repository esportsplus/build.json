...
const read = {
    bundle: (data, filename, path) => {
        ...
        // Bundle Subfolders With Specific Filename
        if (json.bundle || false) {
            json = json.bundle.map(function(subdirectory) {
                return `${subdirectory}/${json.filename || DEFAULT_FILENAME}`;
            });
        }
            ...
            data.assets.forEach((key) => {
                ...
                if (files) {
                    data.output.map[filename][key] = data.output.map[filename][key].concat(
                        toArray(files).map(file => `${directory}/${file}`)
                    );
                }
            });
        }
